...
export default class ThoughtItem extends Component {
    ...
    render() {
        ...
        let repo = store.getRepository(this.props.thought.repository);
        const reponame = repo ? repo.name : ""
        return (
            <TableRow onMouseEnter={() => this.props.onShow(thought)}>
                <td onClick={callback}>{thought.created.toLocaleString()}</td>
                <td onClick={callback}>{thought.name}</td>
                <td onClick={callback}>{reponame}</td>
                <td>
                    {showLayer && layer}
