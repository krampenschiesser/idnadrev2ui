    ...
    if (this.props.asIcon) {
      return (
        <span onMouseEnter={() => this.hover = true} onMouseLeave={() => this.hover = false}>
                    <Icon
                      onClick={() => this.callback(this.props.thought, this.props.store)} style={actionStyle}
                      title={this.title}
                      type={this.icon}/>
                </span>
      );
    } else {
      return <Button icon={this.icon} onClick={() => this.callback(this.props.thought, this.props.store)}>{this.title}</Button>;
    }
  }
}
