...
form.observe({
  ...
  call: ({ form, field, change }) => {
    const pw = form.select("password").get("value");
    if(!pw && change.newValue ) {
      form.select("password").set("value","xxxxxx");
    }
    if(!change.newValue && pw === "xxxxxx") {
      form.select("password").clear()
    }
  },
});
...
export default class RepoCreate extends Component {
  ...
  render() {
    const localCreation = form.select("local").get("value");
          ...
          <Box justify="center" align="center">
            <Form onSubmit={form.onSubmit}>
              ...
              <FormFields>
                ...
                <FormField label={form.$('local').label} error={form.$('local').error}>
                  <FormToggle form={form} name="local"/>
                </FormField>
                {!localCreation &&
