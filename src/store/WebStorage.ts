...
import LocalCryptoStorage, {EncryptedData, Nonce, AuthTag} from "./LocalCryptoStorage";
...
export function toThought(persisted: PersistedThought | undefined, localCrypto: LocalCryptoStorage): Thought | undefined {
  ...
  let decrypt = localCrypto.decrypt(persisted.data, persisted.nonce, persisted.authTag);
...
export default class WebStorage extends Dexie {
  ...
  store<T>(obj: T): Promise<string> {
    ...
    let [encrypted, authTag] = this.localCrypto.encrypt(json, nonce);
